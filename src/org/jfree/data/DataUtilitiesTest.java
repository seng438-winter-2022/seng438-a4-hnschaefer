...
public class DataUtilitiesTest extends DataUtilities {
    ...
    private Mockery mockMutation;
    private Values2D valuesMutation;
    private Mockery mockAllNull;
    private Values2D valuesAllNull;
    private Mockery mockWrong;
    private Values2D valuesWrong;
...
//                 will(returnValue(null));
        ...
        // negative size
        mockMutation = new Mockery();
        valuesMutation = mockMutation.mock(Values2D.class);
        mockMutation.checking(new Expectations() {
          	 {
                 one(valuesMutation).getRowCount();
                 will(returnValue(-1));
                 one(valuesMutation).getColumnCount();
                 will(returnValue(-1));
                 one(valuesMutation).getValue(0, 0);
                 will(returnValue(1));
                 one(valuesMutation).getValue(1, 0);
                 will(returnValue(1));
                 one(valuesMutation).getValue(0, 1);
                 will(returnValue(1));
             }
        });
        
        // elements are null
        mockAllNull = new Mockery();
        valuesAllNull = mockAllNull.mock(Values2D.class); //null
        mockAllNull.checking(new Expectations() {
          	 {
                 one(valuesAllNull).getRowCount();
                 will(returnValue(null));
                 one(valuesAllNull).getColumnCount();
                 will(returnValue(null));
                 one(valuesAllNull).getValue(0, 0);
                 will(returnValue(null));
                 one(valuesAllNull).getValue(1, 0);
                 will(returnValue(null));
                 one(valuesAllNull).getValue(0, 1);
                 will(returnValue(null));
             }
        });
        
        // elements are not doubles
        mockWrong = new Mockery();
        valuesWrong = mockWrong.mock(Values2D.class); //null
        mockWrong.checking(new Expectations() {
          	 {
                 one(valuesWrong).getRowCount();
                 will(returnValue(2));
                 one(valuesWrong).getColumnCount();
                 will(returnValue(2));
                 one(valuesWrong).getValue(0, 0);
                 will(returnValue("a2"));
                 one(valuesWrong).getValue(1, 0);
                 will(returnValue("b3"));
                 one(valuesWrong).getValue(0, 1);
                 will(returnValue("c4"));
             }
        });
    ...
    /* Testing calculate column total after mutation tests
     */
    @Test
    public void calculateColumnTotalMutation() {
        // exercise
        double result = DataUtilities.calculateColumnTotal(valuesMutation, 0);
        // verify
     ...
     */
	@Test
	public void testCalculateColumnTotalOverloaded() {
	   	double result = DataUtilities.calculateColumnTotal(valuesNull, 0, new int[] {});	
    ...
    /* Testing calculate row total after mutation tests
     */
    @Test
    public void calculateRowTotalMutation() {
        // exercise
        double result = DataUtilities.calculateRowTotal(valuesMutation, 0);
        // verify
        assertEquals(result, 0.0, .000000001d);
        // tear-down: NONE in this test method
    }
     ...
     */
	@Test
	public void testCalculateRowTotalOverloaded() {
	   	double result = DataUtilities.calculateRowTotal(valuesNull, 0, new int[] {});	
	    assertEquals(0, result, .000000001d);
	}
    ...
    /* Testing NaN double after mutation tests
     */
    @Test
    public void cloneNaNValue() {
        double[][] testNaNValue = new double[10][10];

        testNaNValue[0][0] = 10.10;
        testNaNValue[1][1] = Double.NaN;
        double[][] result = DataUtilities.clone(testNaNValue);
        assertTrue(DataUtilities.equal(testNaNValue, result));

        testNaNValue[0][0] = 9.9;
        assertFalse(DataUtilities.equal(testNaNValue, result));
    }
    ...
    * * * * */
    ...
    public void createNumberArray2DNull() {
		...
		Number [][] testThreeValues= null;
		try {
			Number [][] result= DataUtilities.createNumberArray2D(nullArray);
		}
		catch (Exception err){
			assertEquals("IllegalArugmentException error should be thrown",IllegalArgumentException.class, err.getClass());
		}
